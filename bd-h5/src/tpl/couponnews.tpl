...
<head>
    ...
    <title>我的资产-_gConfig_.siteTitile</title>
    <link rel="stylesheet" href="css/couponnews.css?@@version">
</head>
<body>
	...
	<header>
		...
		<h1>
			<div id="j-act-title" class="title">我的资产</div>
		</h1>
		<!-- <a href="/" class="home"></a> -->
	</header>
	...
	<div class="container j_wrap">
        ...
        <ul class="news-list coupon-list j_ActionList">
            <script type="text/html" id="j_couponNews">
                <li soda-repeat="$value in list" class="news-item">
                    <div class="info">
                        <p class="item-title">{{$value.title}}</p>
                        <p class="item-content">{{$value.content}}</p>
                    </div>
                    <a href="coupons.html#!/list" class="show-detail">
                        <span>{{$value.notifyTime}}</span><span class="fr btn-detail">查看详情</span>
                    </a>
                </li>
            </script>
        </ul>
	...
	<script type="text/javascript" src="js/_couponnews.js"></script>
