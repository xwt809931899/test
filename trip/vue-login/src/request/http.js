...
axios.interceptors.request.use(
  config => {
    ...
    config.header['Content-Type'] = 'application/x-www-form-urlencoded' //默认使用form表单的格式
    return config
  },
  error => {
    Toast.failed('请求过程出错')
...
export default function plugin (Vue) {
  ...
  })  //defineProperties 原生js自带 直接在一个对象上修改原有属性 或 新增属性，并返回最新的对象
}